...
HugoLunr.prototype.readFile = function (filePath) {
	...
	if (meta.data.subtitle != undefined) {
		subtitle = meta.data.subtitle;
	}

	var location;

	if (meta.data.location != undefined) {
		location = meta.data.location;
	}

	var item = { 'uri': uri, 'title': meta.data.title, 'content': plainText, 'tags': tags, 'location': location, 'subtitle': subtitle };
	self.list.push(item);
}
